...
- `FR-04-T02` / `FR-04-T11` / `FR-04-T12` / `FR-04-T14` / `FR-04-T16` /
  ...
  - `docs-dev/plans/q3a-botlib-aas-port.md` now has all 809 non-template phase
    checklist rows complete. The remaining raw unchecked rows are only the
    reusable checklist gate template.
...
- `FR-04-T12` / `DV-07-T06` In Progress:
  ...
  - Q3A `bot_*` LibVars stay internal to the imported AAS runtime; WORR public policy remains in `sg_bot_*`, with only Q2/WORR `phys_*` and `rs_*` movement/reachability inputs seeded for imported `be_aas_move.c`.
  - `BotRuntimeBuildEntitySnapshot()` now splits BotLib entity snapshots for players, bots, spectators, and monsters/NPCs, and `sg_bot_debug_aas` loaded-status output reports the current counts for those categories.
  ...
  - The latest promotion waves connect bot-attributed damage to the real damage path, health/armor pickup observation to successful item touches, CTF objective pickup/return/capture observations to authoritative gameplay branches, live-aim firing proof, deterministic item-timer policy proof, trace-backed corner-cutting proof, FFA/TDM/coop readiness proof, duel queue/spectator proof, map-restart cleanup proof, warmup bot-start readiness proof, vote bot-exclusion proof, MyMap queue/consume proof, scoreboard bot classification proof, intermission bot cleanup proof, queued nextmap transition proof, map-vote bot-exclusion transition proof, default-off FFA roam-route ownership, default-off FFA spawn-camp avoidance, default-off FFA item-role pickup scoring, default-off FFA role-combat attack ownership, default-off FFA spawn-camp combat avoidance, default-off team role-route ownership, default-off team item-role pickup scoring, default-off team resource-denial pickup scoring, default-off match item-policy umbrella pickup scoring, default-off team friendly-fire attack suppression, default-off team role-combat attack ownership, default-off CTF role-route ownership, default-off CTF role-combat ownership, default-off CTF dropped-flag route ownership, default-off CTF carrier-support route ownership, default-off CTF base-return route ownership, default-off CTF objective-route policy ownership, default-off CTF objective-route precedence over generic role routing, default-off CTF item-role pickup scoring, default-off coop progress-wait, interaction-retry, LeadAdvance, resource-share, anti-blocking, target-sharing, and door/elevator source-hold proofs, and stricter scenario marker checks for live aim plus match-policy evidence. Modes `20` through `51`, `sv_bot_team_policy_smoke` modes `2` and `3`, `sv_bot_warmup_smoke 2`, `sv_bot_vote_smoke 2`, `sv_bot_mymap_smoke 2`, `sv_bot_nextmap_smoke 2`, `sv_bot_mapvote_smoke 2`, `sv_bot_scoreboard_smoke 2`, `sv_bot_intermission_smoke 2`, mode `19` map-change/map-restart rows, plus the coop reuse rows now pass as implemented smoke scenarios.
...
- [ ] `FR-04-T12` Rehost the Quake III Arena BotLib runtime behind a WORR sgame adapter.
  ...
  Runtime lifecycle closeout: BotLib adapter initialization is idempotent for the game-module lifetime, `ShutdownGame()` calls `Bot_RuntimeShutdown()` after level unload/lifecycle status, and Q3A `bot_*` LibVars are documented as internal AAS-runtime state while public WORR policy stays on `sg_bot_*`.
  Entity snapshot closeout: `BotRuntimeBuildEntitySnapshot()` now separates players, bots, spectators, and monsters/NPCs before pushing snapshots into imported `AAS_UpdateEntity`, and loaded `sg_bot_debug_aas` output reports those current snapshot counts.
...
- [ ] `FR-04-T15` Translate Q3A behavior concepts into WORR/Q2 weapons, items, combat, team modes, and architecture boundaries.
  ...
  Latest match item-policy proof update: mode `51` hard-gates the `sg_bot_match_item_policy` umbrella cvar through TDM item-role pickup scoring and deny-enemy resource scoring while the individual proof cvars remain disabled; the implemented scenario catalog now contains 56 short-run rows plus one manual degradation row, and the latest full implemented run passed from `.tmp\bot_scenarios\20260621T210229Z`.
...
- [ ] `DV-07-T06` Maintain imported-source credits and provenance ledgers for the Q3A BotLib and `TTimo/bspc` AAS work.
  ...
  Latest credit/status update: the current 56-row implemented short-run catalog, including the WORR-owned `sg_bot_match_item_policy` mode `51` umbrella pickup-scoring proof, is recorded with no new upstream imports claimed. The latest full suite passed from `.tmp\bot_scenarios\20260621T210229Z`, and the credits ledger records the native status family plus validation evidence.
  Latest FFA item-role credit update: the WORR-owned `sg_bot_ffa_item_roles` pickup scoring bridge, mode `46` scenario promotion, and `ffa_item_role_*` status family are recorded as native work with no new upstream imports claimed.
  Latest CTF item-role credit update: the WORR-owned `sg_bot_ctf_item_roles` pickup scoring bridge, mode `47` scenario promotion, and `ctf_item_role_*` status family are recorded as native work with no new upstream imports claimed; the catalog had 52 implemented short-run rows plus one manual degradation row after that promotion.
  Latest FFA role-combat credit update: the WORR-owned `sg_bot_ffa_role_combat` attack-decision bridge, mode `48` scenario promotion, and `ffa_role_combat_*` status family are recorded as native work with no new upstream imports claimed; the catalog had 53 implemented short-run rows plus one manual degradation row after that promotion.
  Latest team resource-denial credit update: the WORR-owned `sg_bot_team_resource_denial` pickup-scoring bridge, mode `50` scenario promotion, and `team_resource_denial_*` status family are recorded as native work with no new upstream imports claimed.
  Latest match item-policy credit update: the WORR-owned `sg_bot_match_item_policy` umbrella bridge, mode `51` scenario promotion, and objective/nav status ordering fix are recorded as native work with no new upstream imports claimed; the current catalog now has 56 implemented short-run rows plus one manual degradation row.
