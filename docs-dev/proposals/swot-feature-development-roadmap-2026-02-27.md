...
- `FR-04-T02` / `FR-04-T03` / `FR-04-T04` / `FR-04-T05` /
  ...
  - The roadmap defines completion gates for live behavior, combat, items,
    ...
    movement context gap matrix, and the first hazard context gap row are
    complete. The latest full `implemented` catalog now passes 114/114 rows
    from `.tmp\bot_scenarios\implemented_hazard_context\20260628T083945Z`;
    the recommended next implementation slices are accepted
    natural-movement/hazard reference promotion, post-build release acceptance,
    generated Duel/CTF play-depth passes, bot chat user-facing docs readiness,
    and another source-counter variance refresh after the next movement,
    combat, or routing behavior change.
  ...
  - 2026-06-28 hazard context gap round: the catalog now has 114 implemented
    ...
    records the current packaged-map gap as `interaction_world_hazards=0` on
    `base2` until suitable slime/lava or hurt-trigger reference content is
...
- [ ] `FR-04-T15` Translate Q3A behavior concepts into WORR/Q2 weapons, items, combat, team modes, and architecture boundaries.
  ...
  Latest behavior policy umbrella update: modes `52` through `96` now cover the behavior umbrella, profile-driven role/team/item/movement policy, bot chat dispatch/audience/rate/initial/reply/event proofs, live chat event taxonomy, combat/survival depth, multiplayer pacing, coop live/share loops, the `base1` campaign interaction matrix, the movement matrix, movement context rows, accepted teleporter entity-route fallback, and the `base2` hazard context gap. Focused teleporter entity-route validation passed from `.tmp\bot_scenarios\teleporter_entity_route_final\20260629T191851Z`; focused hazard context validation passed from `.tmp\bot_scenarios\movement_hazard_context_gap\20260628T083930Z`, and the latest full `implemented` run passed 114/114 rows from `.tmp\bot_scenarios\implemented_hazard_context\20260628T083945Z`; the next follow-up is accepted natural crouch traversal, accepted hazard traversal/avoidance, fresh post-change source-counter soaks, and fallback behavior.
