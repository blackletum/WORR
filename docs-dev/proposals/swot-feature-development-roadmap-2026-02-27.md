...
- `FR-04-T12` / `DV-07-T06` In Progress:
  ...
  - First behavior/action dispatcher units now compile as WORR-native `bot_actions.*`, `bot_items.*`, and `bot_combat.*` boundaries; `bot_brain.*` samples the dispatcher every accepted command frame, applies validated attack/use decisions through `BotActions_ApplyDecisionDetailed()`, records pending weapon/inventory intents, and emits `q3a_bot_action_status`. Follow-up Phase 4/6 support now adds a per-bot perception blackboard, Q2/WORR weapon metadata, item utility scoring hooks including special-item buckets, health/armor focused routing, combat enemy-fact/damage-attribution proof helpers, opt-in aim/fairness plus live aim-profile/projectile-leading consumption, live pickup and observed-respawn timing consumers, item timer disable/fuzz helper policy, weapon-switch request/observation proof state, deterministic health/armor pickup proof helpers, exact `use_index_only` weapon/inventory dispatch for accepted pending intents, per-bot enemy health/armor estimates from visible observations and split bot-attributed damage deltas, estimate-aware finisher/armor-pressure weapon scoring, carried-weapon inventory scanning, conservative carried non-weapon inventory/powerup use policy, environment utility and sphere deployable policy, placement-checked doppelganger use, last-resort personal teleporter escape policy, safety-gated nuke policy, command-owned nuke retreat routing, generic timed route-goal owner telemetry with nuke retreat, personal teleporter escape, coop leader-route, and coop LeadAdvance consumers, target-source-aware team-objective proof helpers, deterministic role/lane-depth team-policy helpers, FFA/TDM/CTF objective-side match/item/friendly-fire helper policy, coop/resource policy helper metadata, default-off coop WaitForLeader and interaction-retry command owners, a default-off coop resource-share route-selection gate, default-off coop anti-blocking command ownership, and default-off coop monster target-sharing blackboard adoption. Modes `20` through `30` now use those hooks as implemented smoke scenarios, while autonomous team behavior, deeper trigger-aware coop command ownership, door/elevator cooperation, and broader objective intelligence remain future work.
  - Scenario and performance validation tooling now exists under `tools/bot_scenarios/` and `tools/bot_perf/`, covering twenty-two implemented scenario smokes, a manual high-bot soak/degradation policy row, the now-empty default pending scenario set, raw reserved-mode diagnostics, split-marker raw metric merging from frame/blackboard/action/objective/source-counter status lines, embedded proof-marker parsing, field-wise marker metric lookup for split status rows, reserved-mode runtime warmup waiting, hard marker gates for action dispatch, live aim policy, item timer proof, trace-checked corner cutting, FFA/TDM readiness, coop readiness, coop leader routing, coop LeadAdvance, coop progress-wait command ownership, coop interaction retry, coop resource-share route selection, coop anti-blocking command ownership, and coop target-sharing adoption, plus optional discovery for other emerging fields, derived perf budgets, guarded comparison reports, fixture-aware parser tests, analyzer support for optional source-counter fields, import/adapter source-counter getters, split source-counter status emission, source-counter completeness diagnostics, validated bot-frame/route/Q3A-route/static-BSP/entity-clip CPU timing fields, and Q3A memory source-counter reporting.
...
- [ ] `FR-04-T03` Add weapon selection heuristics and situational item use.
  ...
  Door/elevator and trigger-detected coop commands remain follow-up.
