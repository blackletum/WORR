...
- `FR-04-T12` / `DV-07-T06` In Progress:
  ...
  - The latest promotion waves connect bot-attributed damage to the real damage path, health/armor pickup observation to successful item touches, CTF objective pickup/return/capture observations to authoritative gameplay branches, live-aim firing proof, deterministic item-timer policy proof, trace-backed corner-cutting proof, FFA/TDM/coop readiness proof, default-off team role-route ownership, default-off team item-role pickup scoring, default-off team friendly-fire attack suppression, default-off CTF role-route ownership, default-off coop progress-wait, interaction-retry, LeadAdvance, resource-share, anti-blocking, target-sharing, and door/elevator source-hold proofs, and stricter scenario marker checks for live aim plus match-policy evidence. Modes `20` through `35` plus the coop reuse rows now pass as implemented smoke scenarios.
...
- [x] `DV-03-T05` Add bot scenario tests for spawn, navigation, and objective behavior.
  ...
  Progress: `tools/bot_scenarios/run_bot_scenarios.py` now runs implemented bot scenario smokes for spawn-to-item routing, stalled-route recovery, eight-bot reservation pressure, map-change repeat validation, profile-backed spawning from staged botfiles, enemy engagement, weapon switching, health/armor pickup, team objective proof, team-policy duel readiness, aim fairness policy integration, item timer fairness signals, trace-checked corner cutting, FFA/TDM match readiness, coop match readiness, coop leader-route ownership, coop lead-advance route ownership, coop progress-wait command ownership, coop interaction-retry command ownership, coop resource-share route selection, coop anti-blocking command ownership, coop target-sharing adoption, coop door/elevator cooperation, team role route ownership, team item-role pickup scoring, team friendly-fire attack suppression, and CTF role-route ownership. It emits text, JSON, declarative catalog output, Markdown reports, lightweight historical comparisons, pending-gap reports, and semantic marker-backed promotion checks. Server smoke modes `20` through `35`, the coop mode `3` reuse rows, and the coop mode `12` interaction-retry row now validate through frame-command, blackboard, action, objective, nav, match-readiness, coop-readiness, coop-command, and source-counter markers, and the latest implemented run reports 27 passed, 0 failed, 0 timed out, 0 errored, and 0 pending. `tools/bot_scenarios/test_run_bot_scenarios.py` covers parser/report behavior and the promoted-row contracts.
  Latest promotion update: raw diagnostics preserve marker event order and latest source lines, missing metrics report their expected source marker, prior blocked mode `22` diagnostics are superseded by the promoted `health_armor_pickup` row, CTF objective hooks feed the implemented `team_objective` smoke, the default pending rows are promoted into source-backed gates, the follow-on `coop_leader_route` reuse row hard-gates leader-route ownership, mode `27` hard-gates coop LeadAdvance route ownership, mode `28` hard-gates coop resource-share policy plus item reserved-deferral evidence, mode `29` hard-gates close-leader coop anti-blocking command ownership, mode `30` hard-gates support-policy coop target-sharing adoption, mode `31` hard-gates coop door/elevator source ownership plus teammate hold evidence, mode `32` hard-gates TDM role/lane route ownership from live match policy, mode `33` hard-gates TDM item-role pickup scoring from live match item-role policy, mode `34` hard-gates TDM friendly-fire suppression, and mode `35` hard-gates CTF role-route ownership from live match role/lane policy.
