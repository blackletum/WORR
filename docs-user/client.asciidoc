...
cl_worr_native_shadow::
    ...
    NEW-netchan path. The diagnostic observes at most one input command per map
    or native transport epoch; the established movement command remains in
    charge of gameplay. It is not a performance option and does not enable full
    native netcode. Set it before connecting and reconnect after changing it.
