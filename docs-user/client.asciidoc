...
For the default-safe behavior and the current limits of the staged snapshot,
prediction, event, and lag-compensation work, see
link:progressive-networking-controls.md[Progressive Networking Controls].

cg_snapshot_timeline_render::
    Selects the client snapshot timeline comparison mode. This is a
    non-persistent testing control and defaults to 0.
      - 0 — keep established rendering authoritative
      - 1 — audit canonical remote-entity transforms without displaying them
      - 2 — display only parity-proven canonical remote transforms, with an
        independent established-render fallback for every rejected entity

    Mode 2 is intended for controlled playtests. It does not switch the
    predicted local player, enable extrapolation, or promote canonical effects
    and audio.

cg_snapshot_timeline_render_epsilon::
    Maximum per-axis origin, beam-end, or angular difference accepted by the
    snapshot promotion parity gate. Default value is 0.125 and the usable range
    is 0.0001 to 8.0. This is a non-persistent testing control.
...
cl_adaptive_input::
    Enables the experimental adaptive pacing and selective backup policy for
    the existing batched-command path. It uses bounded loss, RTT-change, queue,
    acknowledgement, and rate observations while preserving `cl_maxpackets 0`
    as unlimited. The non-batched compatibility path is unchanged. Default
    value is 0 (disabled).

cl_adaptive_input_status::
    Prints the current adaptive input decision and its telemetry. This command
    is useful for controlled network-impairment tests; it does not enable the
    feature.

cl_worr_native_shadow::
    Enables the client half of a developer-only native command-shadow
    diagnostic. Default value is 0 (disabled). The server must also enable
    `sv_worr_native_shadow`, and the connection must support the compatible
    NEW-netchan path. The diagnostic sends a bounded repeated sample of
    canonical input commands beside the established movement stream, which
    remains in charge of gameplay. It is not a performance option and does not
    enable full native netcode. Set it before connecting and reconnect after changing it.
    An unsupported peer or diagnostic failure silently falls back to the normal
    path. When disabled, normal networking and demo behavior are unchanged.

cl_worr_native_event_shadow::
    Adds the client half of the developer-only native event-stream diagnostic
    to `cl_worr_native_shadow`. Default value is 0 (disabled). The client and
    server must enable both base native-shadow controls and both event-shadow
    controls before connecting. Canonical events are acknowledged only after
    the cgame audit runtime accepts them; established snapshot events still
    produce visible effects and audio. This is not a performance or gameplay
    authority option. Reconnect after changing it. See
    link:progressive-networking-controls.md[Progressive Networking Controls].

cl_snapshot_recovery::
    Enables an experimental recovery policy that can request a full snapshot
    when repeated canonical snapshot checks fail. It uses the existing network
    protocol and leaves the normal automatic recovery from an unreadable frame
    untouched. Default value is 0 (disabled).

cl_snapshot_recovery_debug::
    Prints a message when the experimental recovery policy arms or applies a
    full-snapshot request. Default value is 0 (disabled).

cl_snapshot_recovery_status::
    Prints the current recovery reason, retry/cooldown state, failure streaks,
    and request counters. This is useful for controlled playtests and does not
    enable the feature.

cl_snapshot_shadow_status::
    Prints canonical snapshot projection, publication, and legacy-parity
    counters. A healthy connected session should report an active shadow and
    zero parity mismatches. This command is diagnostic only and does not enable
    canonical rendering or snapshot recovery.
...
cg_weapon_bob::
    Selects first-person weapon bobbing. Default value is 2.
      - 0 - disabled
      - 1 - Quake 3 style
      - 2 - Doom 3 style
    The older `cg_weaponBob` spelling is still accepted as an alias.
...
vk_flare_fade_speed::
    Specifies flare fading effect speed for the Vulkan renderer. Default value
    is 8. Set this to 0 for instant fading.
...
vk_partscale::
    Specifies the minimum size of particles when using the Vulkan renderer.
    Default value is 2.

vk_particle_style::
    Specifies the drawing style of particles when using the Vulkan renderer.
    Default value is 0.
      - 0 — blend colors
      - 1 — saturate colors
...
vk_beam_style::
    Specifies the drawing style of laser beams when using the Vulkan renderer.
    Default value is 0.
      - 0 — textured billboard-type beam (WORR style, very fast to draw)
      - 1 — flat color polygonal beam (original Quake 2 style)
...
- WORR works only with virtual paths constrained to the quake file system.
  ...
  directory root, you should consider moving them into ‘basew/’ to make them
  accessible.
...
- Single player savegame format has been rewritten from scratch for better
  robustness and portability. Only the ‘basew’ game library included in WORR
