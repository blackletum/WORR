...
sv_worr_native_shadow::
    ...
    or native transport epoch while the established movement command remains
    authoritative. It is not a server-performance option and does not enable
    full native netcode. Set it before clients connect and have existing clients
    reconnect after changing it. An unsupported peer or diagnostic failure
