...
static void Cmd_Wait_f(void)
{
    ...
    if (cmd_current->waitCount >= 1000) {
        Com_WPrintfLoc("$cmd_wait_runaway");
        return;
    }
...
void Cbuf_AddText(cmdbuf_t *buf, const char *text)
{
    ...
    if (l > buf->maxsize - buf->cursize) {
        Com_WPrintfLoc("$cmd_overflow", __func__);
        return;
    }
...
// execute the command line
        if (ok) {
            ...
            Com_PrintfLoc("$cmd_line_too_long", MAX_STRING_CHARS);
        }
    }
}
...
void Cmd_Alias_f(void)
{
    ...
    if (Cmd_Argc() < 2) {
        if (LIST_EMPTY(&cmd_alias)) {
            Com_PrintfLoc("$cmd_alias_none");
            return;
        }
        Com_PrintfLoc("$cmd_alias_list_header");
        FOR_EACH_ALIAS(a) {
            Com_PrintfLoc("$cmd_kv_pair", a->name, a->value);
        }
        return;
    }
    ...
    s = Cmd_Argv(1);
    if (Cmd_Exists(s)) {
        Com_PrintfLoc("$cmd_name_already_command", s);
        return;
    }

    if (Cvar_Exists(s, true)) {
        Com_PrintfLoc("$cmd_name_already_cvar_quoted", s);
        return;
    }
...
static void Cmd_UnAlias_f(void)
{
    ...
    while ((c = Cmd_ParseOptions(options)) != -1) {
        ...
        case 'a':
            ...
            List_Init(&cmd_alias);
            Com_PrintfLoc("$cmd_alias_clear_all");
            return;
        default:
            return;
        }
    ...
    if (!cmd_optarg[0]) {
        Com_PrintfLoc("$cmd_alias_missing_name");
        Cmd_PrintHint();
        return;
    }
    ...
    a = Cmd_AliasFind(s);
    if (!a) {
        Com_PrintfLoc("$cmd_undefined_period", s);
        return;
    }
...
static void list_triggers(void)
{
    ...
    if (LIST_EMPTY(&cmd_triggers)) {
        Com_PrintfLoc("$cmd_trigger_none");
        return;
    }

    Com_PrintfLoc("$cmd_trigger_list_header");
    FOR_EACH_TRIGGER(trigger) {
        Com_PrintfLoc("$cmd_kv_pair", trigger->command, trigger->match);
    }
}
...
static void Cmd_Trigger_f(void)
{
    ...
    if (Cmd_Argc() < 3) {
        Com_PrintfLoc("$cmd_trigger_usage", Cmd_Argv(0));
        return;
    }
    ...
    if (matchlen < 4) {
        Com_PrintfLoc("$cmd_trigger_match_too_short");
        return;
    }
...
static void Cmd_UnTrigger_f(void)
{
    ...
    if (Cmd_Argc() == 2) {
        if (!Q_stricmp(Cmd_Argv(1), "all")) {
            ...
            Com_PrintfLoc("$cmd_trigger_removed_count", count, count == 1 ? "" : "s");
    ...
    if (!trigger) {
        Com_PrintfLoc("$cmd_trigger_not_found", command, match);
        return;
    }
...
static void Cmd_If_f(void)
{
    ...
    if (!strcmp(op, "==")) {
        ...
        if (!numeric) {
error:
            Com_PrintfLoc("$misc_cant_use_numeric_expressions", op);
            return;
        }
        ...
        Com_PrintfLoc("$cmd_if_unknown_operator", op);
        Com_PrintfLoc("$cmd_if_valid_operators");
        return;
    }
...
// fail if the macro is a variable name
    if (Cvar_Exists(name, false)) {
        Com_WPrintfLoc("$cmd_name_already_cvar_with_cmd", __func__, name);
        return;
    }
...
// fail if the macro already exists
    ...
    if (macro) {
        if (macro->function != function) {
            Com_WPrintfLoc("$cmd_name_already_defined", __func__, name);
        }
        return;
    }
...
int Cmd_ParseOptions(const cmd_option_t *opt)
{
    ...
    if (*s == '-') {
        ...
        if (p) {
            if (o->sh[1] != ':') {
                Com_PrintfLoc("$cmd_option_no_argument", cmd_argv[cmd_optind]);
                Cmd_PrintHint();
                return '!';
            }
            cmd_optarg = p + 1;
    ...
    if (!p && o->sh[1] == ':') {
        if (cmd_optind + 1 == cmd_argc) {
            Com_PrintfLoc("$cmd_missing_argument", cmd_argv[cmd_optind]);
            Cmd_PrintHint();
            return ':';
        }
        cmd_optarg = cmd_argv[++cmd_optind];
...
unknown:
    Com_PrintfLoc("$cmd_unknown_option", cmd_argv[cmd_optind]);
    Cmd_PrintHint();
    return '?';
}
...
{
    Com_PrintfLoc("$cmd_usage_prefix", cmd_argv[0]);
    while (opt->sh) {
    ...
    if (suffix) {
        Com_PrintfLoc("$cmd_usage_suffix", suffix);
    } else {
        Com_Printf("]\n");
    }
}
...
void Cmd_PrintHelp(const cmd_option_t *opt)
{
    ...
    Com_PrintfLoc("$cmd_options_header");
    while (opt->sh) {
        if (opt->sh[1] == ':') {
...
{
    Com_PrintfLoc("$cmd_help_hint", cmd_argv[0]);
}
...
char *Cmd_MacroExpandString(const char *text, bool aliasHack)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        j = strlen(result);
        if (j >= remaining) {
            Com_PrintfLoc("$cmd_line_expanded_too_long", MAX_STRING_CHARS);
            return NULL;
        }

        if (++count == 100) {
            Com_PrintfLoc("$cmd_macro_loop");
            return NULL;
        }
    ...
    if (inquote) {
        Com_PrintfLoc("$cmd_line_unmatched_quote");
        return NULL;
    }
...
// fail if the command is a variable name
    if (Cvar_Exists(reg->name, false)) {
        Com_WPrintfLoc("$cmd_name_already_cvar_with_cmd", __func__, reg->name);
        return;
    }
...
// fail if the command already exists
    ...
    if (cmd) {
        if (cmd->function) {
            Com_WPrintfLoc("$cmd_name_already_defined", __func__, reg->name);
            return;
        }
...
void Cmd_ExecuteCommand(cmdbuf_t *buf)
{
    ...
    if (!CL_ForwardToServer()) {
        Com_PrintfLoc("$cmd_unknown_command", cmd_argv[0]);
    }
}
...
int Cmd_ExecuteFile(const char *path, unsigned flags)
{
    ...
    Com_PrintfLoc("$cmd_execing_file", path);
...
static void Cmd_Exec_f(void)
{
    ...
    if (Cmd_Argc() != 2) {
        Com_PrintfLoc("$cmd_exec_usage", Cmd_Argv(0));
        return;
    }
...
fail:
    if (ret) {
        Com_PrintfLoc("$cmd_exec_failed", buffer, Q_ErrorString(ret));
    }
}
...
static void Cmd_EchoEx_f(void)
{
    ...
    while ((c = Cmd_ParseOptions(o_echo)) != -1) {
        ...
        case 'l':
            ...
            default:
                Com_PrintfLoc("$cmd_print_level_invalid", cmd_optarg);
                return;
            }
            break;
        case 'n':
...
static void Cmd_List_f(void)
{
    ...
    Com_PrintfLoc("$cmd_list_count", i, total);
}
...
static void Cmd_MacroList_f(void)
{
    ...
    Com_PrintfLoc("$cmd_macro_list_count", i, total);
}
...
static void Cmd_Complete_f(void)
{
    ...
    if (cmd_argc < 2) {
        Com_PrintfLoc("$cmd_complete_usage", cmd_argv[0]);
        return;
    }
...
// fail if the command is a variable name
    if (Cvar_Exists(name, true)) {
        Com_PrintfLoc("$cmd_name_already_cvar", name);
        return;
    }
