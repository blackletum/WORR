#!/usr/bin/env python3
"""Run the headless FR-10-T10 historical moving-brush runtime gate."""
...
SCHEMA = "worr.networking.rewind-mover-runtime.v5"
MAP_NAME = "worr_fr10_rewind_mover"
STATUS_CVAR = "sg_worr_rewind_mover_selftest_status"
...
STATUS_FIELDS = (
    ...
    "rotation_applied",
    "rider_setup",
    "rider_frame_continuity",
    "rider_frame_scene_sealed",
    "rider_provenance_sealed",
    "fixture_reference_blocked",
    ...
    "authority_unchanged",
    "candidate_count",
    "rider_continuity_samples",
    "baseline_fraction_q6",
    "historical_fraction_q6",
    "failure_code",
)
...
def parse_status(text: str) -> dict[str, int | str]:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            "expected exactly one mover runtime status row; "
            f"observed={len(matches)}"
        )
    values = matches[0].group("value").split(":")
    if len(values) != len(STATUS_FIELDS):
        raise RuntimeError(
            "mover runtime status field count changed: "
    ...
    for name, value in zip(STATUS_FIELDS[1:], values[1:], strict=True):
        if not value.isdecimal():
            raise RuntimeError(f"mover runtime status {name} is not decimal: {value!r}")
...
def validate_status(status: dict[str, int | str]) -> dict[str, int | str]:
    if status["status"] != "pass":
        raise RuntimeError(f"mover runtime probe reported {status['status']!r}")
    for name in (
        "setup_ready",
        "scene_ready",
        "rotation_applied",
        ...
        if status[name] != 1:
            raise RuntimeError(f"mover runtime probe did not prove {name}")
    if not isinstance(status["candidate_count"], int) or status["candidate_count"] < 1:
        raise RuntimeError("mover runtime probe found no eligible brush candidate")
    if (
        not isinstance(status["rider_continuity_samples"], int)
        or status["rider_continuity_samples"] < 2
    ):
        raise RuntimeError("mover runtime probe did not retain paired rider frames")
    if status["baseline_fraction_q6"] != 1_000_000:
        raise RuntimeError("current-world baseline was not fully unblocked")
    historical_fraction = status["historical_fraction_q6"]
    if not isinstance(historical_fraction, int) or not 0 < historical_fraction < 500_000:
        raise RuntimeError("historical brush trace did not block in the expected half")
    if status["failure_code"] != 0:
        raise RuntimeError("passing mover runtime probe retained a failure code")
    return status
    ...
    *, command: list[str], working_dir: Path, run_root: Path, timeout: float
    ...
    try:
        ...
        if read_text(stderr_path):
            raise RuntimeError("dedicated mover runtime probe wrote stderr")
        return {
            "status": status,
...
def main() -> int:
    ...
    if not (working_dir / "basew" / "maps" / f"{MAP_NAME}.bsp").is_file():
        parser.error("staged moving-brush fixture is missing from the working directory")
    ...
    try:
        ...
        if any(status != statuses[0] for status in statuses[1:]):
            raise RuntimeError("moving-brush runtime evidence was not deterministic")
        ...
        print(f"rewind mover runtime gate failed: {type(error).__name__}: {error}", file=sys.stderr)
        return 1
    print(output)
    return 0
