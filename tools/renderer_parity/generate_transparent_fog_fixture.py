#!/usr/bin/env python3
"""Generate the deterministic global-fog transparent-world parity map."""
...
from generate_transparent_ordering_fixture import build_bsp


MAP_NAME = "worr_fr01_transparent_fog.bsp"
...
def main() -> int:
    ...
    report = {
        "schema": "worr.renderer-parity.transparent-fog-fixture.v1",
        "outputs": [
            {
                "path": str(path),
                "bytes": len(data),
    ...
    print(json.dumps(report, indent=2, sort_keys=True) if args.json else
          f"{'validated' if args.validate else 'generated'} {len(outputs)} transparent-fog fixture output(s)")
    return 0
